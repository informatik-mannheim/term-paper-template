...
\subsection{Zitate im Text}\label{textquotes}
...
\section*{Dokumentation zu Nutzung generativer KI}
\addcontentsline{toc}{section}{Dokumentation zu Nutzung generativer KI}

\todo[inline]{Ein wichtiges Prinzip wissenschaftlicher Arbeit ist die  Transparenz über die verwendeten Methoden und Werkzeuge zur Gewinnung von Erkenntnissen und zur Erstellung von Texten. Das bedeutet, dass der Einsatz von Sprachmodellen wie Conversational AIs deutlich gemacht werden muss. Deswegen sollten Sie am Ende Ihrer Arbeit die verwendeten Tools und Prompts aufführen. Hier sehen Sie ein paar Bespiele.}

\vspace{1em}
\begin{footnotesize}
  \raggedright
  \textit{Zweck}: Stilkorrektur
  \begin{itemize}
    \item \textit{Tool}: ChatGPT 5.5o
    \item \textit{Prompt}: Bitte lasse den folgenden Text wissenschaftlicher klingen ...
    \item \textit{Teil}: Gesamte Arbeit
    \item \textit{Anmerkungen}: Es wurden nicht alle Vorschläge übernommen.
  \end{itemize}

  \textit{Zweck}: Erstellen einer Grafik
  \begin{itemize}
    \item \textit{Tool}: JumboDumboKI 3.14
    \item \textit{Prompt}: Siehe Listing~\ref{ki_prompt_1}
    \item \textit{Teil}: Abbildung~\ref{fig_sim}
    \item \textit{Anmerkungen}: Grafik musste in Photoshop nachbearbeitet werden.
  \end{itemize}

  \textit{Zweck}: Erstellen einer Datenanalyse
  \begin{itemize}
    \item \textit{Tool}: Claude Code 4.6.344
    \item \textit{Prompt}: Siehe Listing~\ref{ki_prompt_2}
    \item \textit{Teil}: Kapitel~\ref{textquotes}
  \end{itemize}
  ...
  \begin{lstlisting}[float,label={ki_prompt_1},caption={Prompt zur Grafikerstellung},breakindent=0pt]
  Ich habe einen Datensatz mit den Spalten 'Datum' und 'Verkäufe' von 2024 bis 2026. Erstelle mir ein klares Liniendiagramm, um den zeitlichen Verlauf der Verkäufe zu visualisieren. Hebe dabei die Monate mit dem höchsten und dem niedrigsten Umsatz farblich hervor.
  \end{lstlisting}

  \begin{lstlisting}[float,label={ki_prompt_2},caption={Prompt zur Datenanalyse},breakindent=0pt]
  ...
  Context: This data is from my own research.
